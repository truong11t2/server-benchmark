...
func main() {
	...
	http.HandleFunc("/db", func(w http.ResponseWriter, r *http.Request) {
		...
		if len(sortedList) > 0 {
			// Data to be displayed in the template
			data := ListCountries{Countries: sortedList}
		...
		if c.listCountries.ItemCount() > 0 {
			data := c.listCountries.Items()
			for key := range data {
				//fmt.Println("Key:", key, "=>", "Value:", value)
				// Extract data from the query result
				// name, err := json.Marshal(value)
				name, ok := c.read(key)
				if ok {
					//fmt.Println(name)
			...
			// Sort result
			sortedList = sort(results)
		} else {
			// Connect to the PostgreSQL database
			conn, err := pgx.Connect(context.Background(), "postgresql://truong:238103@127.0.0.1:5432/benchmark")
			if err != nil {
			...
			for rows.Next() {
				...
				// Save data to cache
				c.update(columnValue.Id, columnValue.Name)
			}
		}
